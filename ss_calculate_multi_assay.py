...
def main():
    print("[INFO] start...")
    files = sorted(glob.glob("./data/similaritybenchmark/MultiAssay/dataset/*.txt"), key=extract_number)
    uniq_num_multi_set = set()
    for fname in files:
        with open(fname, 'r') as f:
            lines = [l.strip('\n') for l in f.readlines()]
        for l in lines:
            numbers = l.split()
            uniq_num_multi_set.update(map(int, numbers))

    with open('./data/chembl_20.smi', 'r') as f:
        lines = [l.strip('\n').split('\t') for l in f.readlines()]
    used_chembl_ids = uniq_num_multi_set
    id_mol_dict = {}
    for smi, k in lines:
    ...
    result_dict = {
        ...
        "gesim_au": [[] for _ in range(num_use_files)],
    ...
    output_file = 'result/similaritybenchmark/result_multi_assay.pkl'
